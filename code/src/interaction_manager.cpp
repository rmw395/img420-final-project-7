	...
	while (file_name != "") {
		if (!dir->current_is_dir()) {
			...
			if (loaded_recipe.is_valid()) {
				for (int i = 0; i < loaded_recipe->input_effects.size(); i++) {
					String effect_id = loaded_recipe->input_effects[i];
...
void InteractionManager::add_status_effect(Entity* entity, String effect_id, double duration = 0) {
    ...
    for (int i = 0; i < available_recipes_variant.size(); i++) {
        ...
        Array& input_effects = recipe->input_effects;
        for (int j = 0; j < input_effects.size(); j++) {
            String required_effect_id = input_effects[j];
            if (!entity_status_map.has(required_effect_id)) {
    ...
    if (candidate_recipes.size() > 0) {
        ...
        for (int i = 0; i < best_recipe->input_effects.size(); i++) {
            String consumed_id = best_recipe->input_effects[i];
            if (i < best_recipe->consumed_effects.size() && (bool)best_recipe->consumed_effects[i] && consumed_id != effect_id) {
        ...
        for (int i = 0; i < best_recipe->output_effects.size(); i++) {
            add_status_effect(entity, best_recipe->output_effects[i]);
        }
    }
    else {
